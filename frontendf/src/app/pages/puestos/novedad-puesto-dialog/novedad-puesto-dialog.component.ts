...
export class NovedadPuestoDialogComponent implements OnInit {
  ...
  private nombreUsuarioActual(): string {
    const u = this.authService.getUserFromStorage() || {};
    const full = (u.full_name || '').toString().trim();
    if (full) return full;
    const compuesto = `${u.first_name || ''} ${u.last_name || ''}`.trim();
    return compuesto || (u.username || '').toString();
  }
  ...
  guardar(): void {
    ...
    // En Modificación, el "Tipo" (resumen editado) viaja en el modelo; el backend lo
    // aplica al horario del puesto al registrar la novedad.
